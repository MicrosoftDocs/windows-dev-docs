...
:::row:::
    ...
    :::column span="2":::
        ...
        - [**.NET 8+** supports native Arm64 execution](/dotnet/core/whats-new/dotnet-8). To develop .NET apps that run natively on Arm64 devices, install Arm-native Visual Studio and the [.NET 8](https://dotnet.microsoft.com/en-us/download/dotnet/8.0) or [.NET 9](https://dotnet.microsoft.com/en-us/download/dotnet/9.0) Arm64 SDK. (Existing x64 .NET apps also run on Arm devices under Windows OS-level x64 emulation.) Learn more about .NET support for Arm on the .NET Blog: **[Arm64 Performance Improvements in .NET 8 (Oct 2023)](https://devblogs.microsoft.com/dotnet/this-arm64-performance-in-dotnet-8)**.
        - **.NET Arm64 SDK:** By default, if you `dotnet run` a .NET app with the Arm64 SDK, it runs as Arm64. Run `dotnet --info` to see the runtime environment details.
        ...
        - **[GitHub Actions](https://github.com/features/actions)**: GitHub Actions, GitHub's CI/CD workflow engine, is an integral part of many developers' workflows. They use this tool to continuously build, test, and deploy apps. GitHub Actions is now available for Windows on Arm in two flavors: 1. **[self-hosted runners](https://github.blog/changelog/2022-09-28-github-actions-self-hosted-runners-now-support-windows-arm-hardware/)** that you can host on an Arm VM or Arm device, and 2. **[GitHub-hosted runners](https://github.blog/changelog/2023-10-30-accelerate-your-ci-cd-with-arm-based-hosted-runners-in-github-actions/)** (now generally available).
        ...
        - **[Windows on Snapdragon (WoS) Porter by Qualcomm](https://github.com/qualcomm/extension-wos-porter/)**: An open-source VS Code extension that integrates with GitHub Copilot Chat to automatically port x64 Windows applications to native Arm64. Give it a GitHub repository URL and it analyzes x64-specific code, ports build systems (CMake, MSBuild, Meson, and more), translates x86 SIMD intrinsics to NEON, iteratively builds and validates the Arm64 output, and generates a ready-to-apply git patch. Learn more at the [Qualcomm Windows on Snapdragon AI developer page](https://www.qualcomm.com/developer/windows-on-snapdragon/windows-on-snapdragon-ai).
    :::column-end:::
:::row-end:::
...
- [Qualcomm Windows on Snapdragon AI developer tools](https://www.qualcomm.com/developer/windows-on-snapdragon/windows-on-snapdragon-ai)
