...
Arm-based devices are becoming increasingly popular due to their power-frugal nature, longer battery life, and impressive processing power. Windows on Arm supports Neural Processing Units (NPUs) tuned for the increasingly popular AI and Machine Learning workloads. 

This guide covers the steps for adding support to your Windows apps for devices powered by Arm64 processors. It also provides guidance on ways to address potential issues or blockers, such as third-party dependencies or plug-ins, that might interfere with creating an Arm64-based version of your app.
...
While the ability to emulate x64 and x86 on Arm devices is a great step forward, this guide helps you add Arm-native support so that your app can take advantage of native performance gains and the unique qualities of Arm64-powered devices, including:
...
Additionally, [Kernel drivers](/windows-hardware/drivers/kernel/) need to be built as native Arm64. The kernel doesn't include emulation. This limitation primarily affects virtualization scenarios. For apps that use device drivers requiring direct access to the internals of the OS or hardware running in kernel mode, rather than user mode, and that aren't yet updated to support Arm64 processors, see [Building Arm64 Drivers with the WDK](/windows-hardware/drivers/develop/building-arm64-drivers).

> [!NOTE]
> Progressive Web Apps (PWAs) already execute with native Arm64 performance.
...
Consider several factors when choosing between cross compilation or native compilation, such as available hardware and simplicity of test execution.
...
To update your app to run natively on Arm64 devices:
...
After you confirm that your app is optimized for Arm devices:
...
*(Optional)*: To verify that your app binary is built for Arm64, open your project directory in PowerShell (right-click your app project in Visual Studio Solution Explorer and select **Open in Terminal**). Change directories so that your project's new `bin\ARM64\Debug` or Release directory is selected. Enter the command: `dumpbin /headers .\<appname>.exe` (replacing `<appname>` with the name of your app). Scroll up in your terminal's output results to find the `FILE HEADER VALUES` section and confirm the first line is `AA64 machine (ARM64)`.
...
1. Change the build setting from **Debug** to **Release**.
1. In the **Build** drop-down menu, select **Rebuild Solution** and wait for the project to rebuild.
1. You receive a **Rebuild All succeeded** output. If not, see the [Troubleshooting](#troubleshooting) section.

Once the binaries are built for your app to support Arm64, you want to test them. That testing requires having a device or a virtual machine running Windows on Arm.  

If you're developing on a Windows on Arm device, then you have an easy setup with Visual Studio local debugging. If you're cross-compiling (using a device that isn't running on an Arm processor), then you want to use remote debugging on a Windows on Arm device or a virtual machine to enable your development experience in Visual Studio while running the Arm64 app on another device.

### Windows on Arm hardware or virtual machines available for testing

If you're looking for hardware to use for continuous integration (CI) and testing, these are a few of the Windows devices with an Arm64-based processor:
...
Adding a test automation pass is an important consideration for your Continuous Integrations and Continuous Delivery (CI/CD) strategy. For Arm64 solutions running on Windows, it's important to run your test suite on Arm64 architecture. This architecture could be actual Windows on Arm hardware, using one of the Arm devices listed earlier, or a Virtual Machine, from the VMs listed earlier.

Compiling the app is more convenient when done on the same machine as the tests, but in many cases isn't required. Instead, you can consider extending the existing build infrastructure to produce a cross-compiled output for Arm64.

## Step 4 - Update your installer and publish your updated app

If you publish to the Microsoft Store, once you build an Arm64 version of your app by following the preceding steps, you can update your existing app package in the Microsoft Store by visiting your [Partner Center dashboard](https://partner.microsoft.com/dashboard) and adding the newly built ARM64 binaries to the submission.

If you don't already publish your app in the Microsoft Store, you can follow the instructions to [create an app submission](/windows/apps/publish/publish-your-app/create-app-submission?pivots=store-installer-msi-exe) based on whether you want to submit an MSI or EXE, MSIX package, PWA, or App add-on.

If you build your own installer, you should ensure it can install your new Arm64 version successfully. Most installer frameworks, such as [WiX](https://wixtoolset.org/docs/releasenotes/), [Squirrel](https://github.com/Squirrel/Squirrel.Windows), [InnoSetup](https://github.com/jrsoftware/issrc), [InstallAware](https://www.installaware.com/mh52/desktop/set64bitmode.htm), and others support Windows on Arm without issue.
...
Now that you publish an Arm64 version of your app, you want to ensure that it stays updated the same way as other versions of your app. Keep versions and features aligned across architectures to avoid customer confusion in the future.

## Troubleshooting

Common issues that interfere with or block you from adding an Arm64 version of your existing x64 or x86 Windows app include:
...
If you can't build your app because of a dependency, whether internal, from a third party, or from an open-source library, you need to either update that dependency to support ARM64 architecture or remove it.

- For internal dependencies, rebuild the dependency for ARM64 support.

- For third-party dependencies, request the maintainer to rebuild the dependency with ARM64 support.

- For open source dependencies, check [vcpkg](https://vcpkg.io/en/packages.html) to see if a newer version of the dependency includes ARM64 support that you can update to. If no update exists, consider contributing the addition of ARM64 support to the package yourself. Many open source maintainers appreciate the contribution.

- The Linaro organization works with businesses and open source communities to develop software on Arm-based technology. You can [file a request with the Linaro Service Desk to help update package support](https://linaro-servicedesk.atlassian.net/servicedesk/customer/portal/22) for any missing dependencies related to Windows on Arm.

- Use [Arm64EC](arm64ec.md). Arm64EC versions of dependencies can be used to rebuild an application while still utilizing x64 versions of dependencies. Any x64 code, including code from dependencies, in an Arm64EC process runs under emulation in your app. (Arm64 versions of dependencies aren't usable in this case.)

- Remove and replace the dependency on your app project.

### Code is written for a specific architecture other than Arm64

- You need to modify CPU-specific assembly or inline intrinsic function calls to match available instructions and functions on the Arm CPU. For guidance, see: [Using Assembly and Intrinsics in C or C++ Code](https://developer.arm.com/documentation/100748/0620/Using-Assembly-and-Intrinsics-in-C-or-C---Code).

### Your app relies on a kernel driver

[Kernel drivers](/windows-hardware/drivers/kernel/) must be built as native Arm64. The kernel doesn't provide any emulation. This limitation mainly affects virtualization scenarios. For apps that use device drivers requiring direct access to the internals of the OS or hardware running in kernel mode (rather than user mode), and that aren't yet updated to support Arm64 processors, see [Building Arm64 Drivers with the WDK](/windows-hardware/drivers/develop/building-arm64-drivers).

Additionally, [drivers on Windows](/windows-hardware/drivers/gettingstarted/) must be built as Arm64 and can't be emulated. For apps that rely on software drivers that aren't yet updated to support Arm64 processors, see [Building Arm64 Drivers with the WDK](/windows-hardware/drivers/develop/building-arm64-drivers).
...
- [clang-cl](https://clang.llvm.org/docs/MSVCCompatibility.html)  compiles C++ code for Windows and can serve as a replacement for the MSVC compiler and linker. It still uses headers and libraries from MSVC and is ABI-compatible with MSVC.

As well as third-party frameworks, including:
