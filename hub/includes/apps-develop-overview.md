---
ms.topic: include
ms.date: 07/12/2024
---

:::row:::
    :::column:::
       [![Features icon](../apps/develop/images/tile-features.png)](../apps/develop/accessibility.md)<br>
        **[Features](../apps/develop/accessibility.md)**<br>
        Explore the building blocks for creating rich Windows experiences.
    :::column-end:::
    :::column:::
        [![Modernization icon](../apps/develop/images/tile-modernization.png)](../apps/desktop/modernize/index.md)<br>
        **[Use features with other UI frameworks](../apps/desktop/modernize/index.md)**<br>
       Learn how to use Windows platform features outside of WinUI.
    :::column-end:::
    :::column:::
       [![Platform icon](../apps/develop/images/tile-platforms.png)](../apps/develop/platform/csharp-winrt/index.md)<br>
        **[Platform](../apps/develop/platform/csharp-winrt/index.md)**<br>
        Deep-dive into technologies like XAML and WinRT.
    :::column-end:::
:::row-end:::
:::row:::
    :::column:::
       [![API reference icon](../apps/develop/images/tile-apireference.png)](../apps/api-reference/index.md)<br>
        **[API reference](../apps/api-reference/index.md)**<br>
        Look up detailed API docs for every namespace and feature.
    :::column-end:::
    :::column:::
        [![Tutorials icon](../apps/develop/images/tile-tutorials.png)](../apps/winui/winui3/desktop-winui3-app-with-basic-interop.md)<br>
        **[Tutorials](../apps/winui/winui3/desktop-winui3-app-with-basic-interop.md)**<br>
       Follow step-by-step guides to learn by building.
    :::column-end:::
    :::column:::
    :::column-end:::
:::row-end:::