...
**PlaceholderText** is displayed inside the NumberBox and will only appear when **Value** is set to _NaN_ or when the input is cleared by the user.
...
When a NumberBox is cleared of input, **Value** will be set to _NaN_ to indicate no numerical value is present.
