...
Another possible technique is to define a date that's available as a data object or in the data context, then set the date property as a XAML attribute that references a [{Binding} markup extension](../../platform/xaml/binding-markup-extension.md) that can access the date as data.
