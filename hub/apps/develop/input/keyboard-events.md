...
You can also call a control's [**Focus**](/windows/windows-app-sdk/api/winrt/microsoft.ui.xaml.controls.control) method to force focus. This is necessary when you implement shortcut keys, because keyboard focus is not set by default when your UI loads. For more info, see the **Shortcut keys example** later in this topic.

For a control to receive input focus, it must be enabled, visible, and have [**IsTabStop**](/windows/windows-app-sdk/api/winrt/microsoft.ui.xaml.controls.control) and [**HitTestVisible**](/windows/windows-app-sdk/api/winrt/microsoft.ui.xaml.uielement.ishittestvisible) property values of **true**. This is the default state for most controls. When a control has input focus, it can raise and respond to keyboard input events as described later in this topic. You can also respond to a control that is receiving or losing focus by handling the [**GotFocus**](/windows/windows-app-sdk/api/winrt/microsoft.ui.xaml.uielement.gotfocus) and [**LostFocus**](/windows/windows-app-sdk/api/winrt/microsoft.ui.xaml.uielement.lostfocus) events.

By default, the tab sequence of controls is the order in which they appear in the Extensible Application Markup Language (XAML). However, you can modify this order by using the [**TabIndex**](/windows/windows-app-sdk/api/winrt/microsoft.ui.xaml.controls.control) property. For more info, see [Implementing keyboard accessibility](/previous-versions/windows/apps/hh868161(v=win.10)).
