...
description: Incorporate speech recognition and speech synthesis into your Windows apps for accessible, hands-free input and natural voice control.
...
## Key APIs and namespaces

Speech functionality in Windows apps uses the following Windows Runtime APIs:

| Namespace | Purpose |
| --- | --- |
| [**Windows.Media.SpeechRecognition**](/uwp/api/windows.media.speechrecognition) | Speech recognition: converting spoken input to text, supporting both predefined and custom grammars |
| [**Windows.Media.SpeechSynthesis**](/uwp/api/windows.media.speechsynthesis) | Text-to-speech: converting text strings or SSML markup to spoken audio output |

Key classes:

- [**SpeechRecognizer**](/uwp/api/windows.media.speechrecognition.speechrecognizer) — The primary class for speech recognition. Create an instance, add constraints (grammars), compile, and then call recognition methods.
- [**SpeechRecognitionListConstraint**](/uwp/api/windows.media.speechrecognition.speechrecognitionlistconstraint) — Define a list of words or phrases for command recognition.
- [**SpeechRecognitionTopicConstraint**](/uwp/api/windows.media.speechrecognition.speechrecognitiontopicconstraint) — Use a predefined grammar (dictation or web search).
- [**SpeechSynthesizer**](/uwp/api/windows.media.speechsynthesis.speechsynthesizer) — The primary class for text-to-speech. Generates an audio stream from text.

> [!NOTE]
> These are Windows Runtime APIs in the `Windows.Media` namespace. They are available to both UWP and WinUI 3 desktop apps with no additional NuGet packages beyond the Windows App SDK. Speech recognition requires microphone access — your app must declare the **Microphone** capability in its manifest.

## Basic speech recognition

The following example shows how to recognize a single spoken phrase using a predefined dictation grammar. This code runs in a WinUI 3 or UWP app.

```csharp
using Windows.Media.SpeechRecognition;

// Create a speech recognizer with the default language
var speechRecognizer = new SpeechRecognizer();

// Add a dictation constraint (recognizes free-form speech)
...
{
    // Start recognition and wait for the user to speak
    SpeechRecognitionResult result = await speechRecognizer.RecognizeAsync();

    if (result.Status == SpeechRecognitionResultStatus.Success)
    {
        // result.Text contains the recognized speech
        myTextBlock.Text = result.Text;
    }
}
...
### Command recognition with a word list

To recognize specific commands rather than free-form dictation, use a [**SpeechRecognitionListConstraint**](/uwp/api/windows.media.speechrecognition.speechrecognitionlistconstraint):
...
speechRecognizer.Constraints.Add(
    new SpeechRecognitionListConstraint(
        new[] { "start", "stop", "pause", "resume", "go back" },
        "commands"));
...
For scenarios where you want to listen continuously (such as dictation or always-on voice commands), use the [**ContinuousRecognitionSession**](/uwp/api/windows.media.speechrecognition.speechcontinuousrecognitionsession):
...
{
    // This fires on a background thread — use DispatcherQueue to update UI.
    // In a Page or Window code-behind, capture the queue first:
    // var dispatcherQueue = this.DispatcherQueue;
    // Then use it in the handler:
    dispatcherQueue.TryEnqueue(() =>
    {
        myTextBlock.Text += e.Result.Text + " ";
    });
};
...
The following example shows how to synthesize speech from text and play it using a [**MediaPlayerElement**](/windows/windows-app-sdk/api/winrt/microsoft.ui.xaml.controls.mediaplayerelement):
...
// Generate an audio stream from text
SpeechSynthesisStream stream =
    await synthesizer.SynthesizeTextToStreamAsync(
        "Hello! This is a speech synthesis example.");

// Play the audio through a MediaPlayerElement defined in XAML
...
To control voice characteristics such as pitch, rate, and volume, use SSML markup with [**SynthesizeSsmlToStreamAsync**](/uwp/api/windows.media.speechsynthesis.speechsynthesizer.synthesizessmltostreamasync):
...
SpeechSynthesisStream stream =
    await synthesizer.SynthesizeSsmlToStreamAsync(ssml);
```
...
- List of words or phrases, or an SRGS grammar file
  - The **Listening** screen.
...
The following images show an example of the built-in recognition flow for a speech recognizer that uses an SRGS-defined constraint. In this example, speech recognition is successful.

![Initial recognition screen for a constraint based on an SRGS grammar file](images/speech/speech-listening-initial.png)

![Intermediate recognition screen for a constraint based on an SRGS grammar file](images/speech/speech-listening-intermediate.png)

![Final recognition screen for a constraint based on an SRGS grammar file](images/speech/speech-listening-complete.png)
...
- An SRGS grammar is a static document that, unlike a programmatic list constraint, uses the XML format defined by the [SRGS Version 1.0](https://www.w3.org/TR/speech-grammar/). An SRGS grammar provides the greatest control over the speech recognition experience by letting you capture multiple semantic meanings in a single recognition.
  ...
  - Use the [sapi:subset](/previous-versions/office/developer/speech-technologies/jj572474(v=office.14)) element to help match speech input. This element is a Microsoft extension to the SRGS specification to help match partial phrases. (This is a reference to legacy SAPI documentation.)
...
For a small list of words and phrases, or a list of infrequently used words and phrases, create custom pronunciations in an SRGS grammar. See [token Element](/previous-versions/office/developer/speech-technologies/hh361600(v=office.14)) for more info. (Legacy SAPI reference.)

For larger lists of words and phrases, or frequently used words and phrases, create separate pronunciation lexicon documents. See [About Lexicons and Phonetic Alphabets](/previous-versions/office/developer/speech-technologies/hh361646(v=office.14)) for more info. (Legacy SAPI reference.)
...
You can address both issues by using SSML instead of plain text as input to the speech synthesizer. For more info about SSML, see [Use SSML to Control Synthesized Speech](/previous-versions/office/developer/speech-technologies/hh378454(v=office.14)) and [Speech Synthesis Markup Language Reference](/previous-versions/office/developer/speech-technologies/hh378377(v=office.14)). (These are legacy SAPI references; the SSML markup format itself remains current and is supported by the Windows speech APIs.)

## Samples

- [Speech recognition and speech synthesis sample (UWP)](https://github.com/Microsoft/Windows-universal-samples/tree/master/Samples/SpeechRecognitionAndSynthesis) — This is a UWP sample. The underlying `Windows.Media.SpeechRecognition` and `Windows.Media.SpeechSynthesis` APIs are also available in WinUI 3 desktop apps, but the UI code (XAML namespaces, windowing) requires adaptation for WinUI 3.
