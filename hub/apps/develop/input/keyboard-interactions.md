...
For a control to be included in the tab order, the [IsEnabled](/windows/windows-app-sdk/api/winrt/microsoft.ui.xaml.controls.control.isenabled) property must be set to **true** and the [IsTabStop](/windows/windows-app-sdk/api/winrt/microsoft.ui.xaml.uielement.istabstop) property must be set to **true**.

To specifically exclude a control from the tab order, set the [IsTabStop](/windows/windows-app-sdk/api/winrt/microsoft.ui.xaml.uielement.istabstop) property to **false**.
...
You can override the default tab order by setting the [TabIndex](/windows/windows-app-sdk/api/winrt/microsoft.ui.xaml.uielement.tabindex) property.
...
For WinUI apps, initial focus is set to the element with the highest [TabIndex](/windows/windows-app-sdk/api/winrt/microsoft.ui.xaml.uielement.tabindex) that can receive focus. Child elements of container controls are ignored. In a tie, the first element in the visual tree receives focus.
