...
> [!NOTE]
> To read how [XamlLight](/windows/windows-app-sdk/api/winrt/microsoft.ui.xaml.media.xamllight) objects apply [CompositionLights](/windows/windows-app-sdk/api/winrt/microsoft.ui.composition.compositionlight) to illuminate XAML UIElements, see [XAML lighting](xaml-lighting.md).
...
[CompositionLight](/windows/windows-app-sdk/api/winrt/microsoft.ui.composition.compositionlight) allows you to create various lights and place them in coordinate space. These lights target visuals that you wish to identify as lit by the light.

### Light Types

| Type | Description |
| --- | --- |
| [AmbientLight](/windows/windows-app-sdk/api/winrt/microsoft.ui.composition.ambientlight) | A light source that emits non-directional light that appears reflected by everything in the scene. |
| [DistantLight](/windows/windows-app-sdk/api/winrt/microsoft.ui.composition.distantlight) | An infinitely large distant light source that emits light in a single direction. Like the sun. |
| [PointLight](/windows/windows-app-sdk/api/winrt/microsoft.ui.composition.pointlight) | A point source of light that emits light in all directions. Like a lightbulb. |
| [SpotLight](/windows/windows-app-sdk/api/winrt/microsoft.ui.composition.spotlight) | A light source that emits inner and outer cones of light. Like a flashlight. |

## Targets

When lights target a Visual (add to [Targets](/windows/windows-app-sdk/api/winrt/microsoft.ui.composition.compositionlight.targets) list), the Visual and all its descendants are aware of and respond to this light source. This can be something as simple as setting a PointLight source at the root of a tree and all visuals below react to the animation of the point light's direction.
...
**Lifetime** | CompositionLight may experience lifetime conditions (example: the garbage collector may recycle the light object before it is used). We recommend keeping a reference to your lights by adding lights as a member to help the application manage lifetime.
...
**Targets and Coordinate Space** | CoordinateSpace is the visual space in which all the light's properties must be set. CompositionLight.Targets must be within the CoordinateSpace tree.
...
> [!NOTE]
> When multiple lights hit the same Visual, or whenever a light's color value gets large enough to exceed 1.0, the color of the light may change because of clamping of a light's color channel.
...
[SceneLightingEffect](/windows/windows-app-sdk/api/winrt/microsoft.ui.composition.effects.scenelightingeffect) is used to modify the default lighting applied to the contents of a [SpriteVisual](/windows/windows-app-sdk/api/winrt/microsoft.ui.composition.spritevisual) targeted by a [CompositionLight](/windows/windows-app-sdk/api/winrt/microsoft.ui.composition.compositionlight).

[SceneLightingEffect](/windows/windows-app-sdk/api/winrt/microsoft.ui.composition.effects.scenelightingeffect) is frequently used for material creation. A SceneLightingEffect is an effect used when you want to achieve something more complex, like enabling reflective properties on an image and/or providing an illusion of depth with a normal map. A SceneLightingEffect provides the ability to customize your UI by using the lighting properties like specular and diffuse amounts. You can further customize lighting effects with the rest of the effects pipeline allowing you individually to blend and compose different lighting reactions with your content.
...
Property | Description
--- | ---
**Normal Map** | NormalMaps create an effect of a texture where a normal pointing towards the light will be brighter and a normal pointing away will be dimmer. To add a NormalMap to your targeted visual use a [CompositionSurfaceBrush](/windows/windows-app-sdk/api/winrt/microsoft.ui.composition.compositionsurfacebrush) using LoadedImageSurface to load a NormalMap asset.
**Ambient** | Ambient properties are mostly used to control the overall color reflection.
**Specular** | Specular reflection creates highlights on objects, making them appear shiny. You can control the level of specular reflection as well as the level of shine. These properties are manipulated to create material effects like shiny metals or glossy paper.
**Diffuse** | Diffused reflection scatters the light in all directions.
**Reflectance Model** | [Reflectance Model](/windows/windows-app-sdk/api/winrt/microsoft.ui.composition.effects.scenelightingeffectreflectancemodel) allows you to choose between [Blinn Phong](/visualstudio/designers/how-to-create-a-basic-phong-shader) and Physically Based Blinn Phong. You would choose Physically Based Blinn Phong when you want to have condensed specular highlights.
