...
The net result is higher user engagement and retention with your app.

## Balancing Control and Dynamism

In traditional UI, [KeyFrameAnimation](/windows/windows-app-sdk/api/winrt/microsoft.ui.composition.keyframeanimation)s are the predominant way to describe motion. KeyFrames provided the utmost control to designers and developers to define the start, end, and interpolation. Although this is very useful in many cases, KeyFrame Animations are not very dynamic; the motion is not adaptive and looks the same under any condition.
...
[NaturalMotionAnimation](/windows/windows-app-sdk/api/winrt/microsoft.ui.composition.naturalmotionanimation)s exist to help bridge this divide – enabling a balance of control for the important elements of an animation like start/finish, but maintaining motion that looks and feels natural and dynamic.
...
In WinUI and the Windows App SDK, you have access to a natural motion experience: **spring animations**. See [Spring animations](spring-animations.md) for a more in depth walkthrough of springs.
