...
C#/WinRT currently provides support for consuming WinRT APIs through the use of [Target Framework Monikers](../../../desktop/modernize/winrt-apis-desktop-apps.md) (TFMs) in .NET. Setting the TFM with a specific Windows SDK version adds references to the Windows SDK projection and runtime assemblies generated by C#/WinRT.
...
- Error	CS0246: The type or namespace name 'Windows' could not be found (are you missing a using directive or an assembly reference?)
  ...
  ```
  Refer to the docs on [Calling Windows Runtime APIs](../../../desktop/modernize/winrt-apis-desktop-apps.md) for more details on specifying the `<TargetFramework>` property.
