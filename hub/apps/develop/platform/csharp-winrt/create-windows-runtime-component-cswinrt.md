...
1. Update the `TargetFramework` in the **AuthoringDemo.csproj** file and add the following elements to the `PropertyGroup`:
    ...
    To access Windows Runtime types, you need to set a specific Windows SDK version in the TFM. For more details on the supported version, see [.NET 6 and later: Use the TFM option](../../../desktop/modernize/desktop-to-uwp-enhance.md#net-6-use-the-target-framework-moniker-option).
