...
The move and draw command syntax is supported by an internal type converter for XAML, which parses the commands and produces a run-time graphics representation. This representation is basically a finished set of vectors that is ready for presentation. The vectors themselves don't complete the presentation details; you'll still need to set other values on the elements. For a [**Path**](/windows/windows-app-sdk/api/winrt/microsoft.ui.xaml.Shapes.Path) object you also need values for [**Fill**](/windows/windows-app-sdk/api/winrt/microsoft.ui.xaml.Shapes.Shape.Fill), [**Stroke**](/windows/windows-app-sdk/api/winrt/microsoft.ui.xaml.shapes.shape.stroke), and other properties, and then that **Path** must be connected to the visual tree somehow. For a [**PathIcon**](/windows/windows-app-sdk/api/winrt/microsoft.ui.xaml.Controls.PathIcon) object, set the [**Foreground**](/windows/windows-app-sdk/api/winrt/microsoft.ui.xaml.controls.iconelement.foreground) property.

There are two properties in the Windows Runtime that can use a string representing move and draw commands: [**Path.Data**](/windows/windows-app-sdk/api/winrt/microsoft.ui.xaml.shapes.path.data) and [**PathIcon.Data**](/windows/windows-app-sdk/api/winrt/microsoft.ui.xaml.controls.pathicon.data). If you set one of these properties by specifying move and draw commands, you typically set it as a XAML attribute value along with other required attributes of that element. Without getting into the specifics, here's what that looks like:
...
For Windows Runtime XAML, the move and draw commands produce a [**PathGeometry**](/windows/windows-app-sdk/api/winrt/microsoft.ui.xaml.Media.PathGeometry) with a single [**PathFigure**](/windows/windows-app-sdk/api/winrt/microsoft.ui.xaml.Media.PathFigure) object with a [**Figures**](/windows/windows-app-sdk/api/winrt/microsoft.ui.xaml.media.pathgeometry.figures) property value. Each draw command produces a [**PathSegment**](/windows/windows-app-sdk/api/winrt/microsoft.ui.xaml.Media.PathSegment) derived class in that single **PathFigure**'s [**Segments**](/windows/windows-app-sdk/api/winrt/microsoft.ui.xaml.media.pathfigure.segments) collection, the move command changes the [**StartPoint**](/windows/windows-app-sdk/api/winrt/microsoft.ui.xaml.media.pathfigure.startpoint), and existence of a close command sets [**IsClosed**](/windows/windows-app-sdk/api/winrt/microsoft.ui.xaml.media.pathfigure.isclosed) to **true**. You can navigate this structure as an object model if you examine the **Data** values at run time.
...
1. Specify exactly one move command.
1. Specify one or more draw commands.
1. Specify a close command. You can omit a close command , but that would leave your figure open (that's uncommon).
...
Creates a straight line between the current point and the specified end point. `l 20 30` and `L 20,30` are examples of valid line commands. Defines the equivalent of a [**LineGeometry**](/windows/windows-app-sdk/api/winrt/microsoft.ui.xaml.Media.LineGeometry) object.
...
Creates an elliptical arc between the current point and the specified end point. Defines the equivalent of a [**PathGeometry**](/windows/windows-app-sdk/api/winrt/microsoft.ui.xaml.Media.PathGeometry) with an [**ArcSegment**](/windows/windows-app-sdk/api/winrt/microsoft.ui.xaml.Media.ArcSegment).
...
Using the **Pen** tool and other drawing tools in Blend for Microsoft Visual Studio 2015 will usually produce a [**Path**](/windows/windows-app-sdk/api/winrt/microsoft.ui.xaml.Shapes.Path) object, with move and draw commands.

You might see existing move and draw command data in some of the control parts defined in the Windows Runtime XAML default templates for controls. For example, some controls use a [**PathIcon**](/windows/windows-app-sdk/api/winrt/microsoft.ui.xaml.Controls.PathIcon) that has the data defined as move and draw commands.

There are exporters or plug-ins available for other commonly used vector-graphics design tools that can output the vector in XAML form. These usually create [**Path**](/windows/windows-app-sdk/api/winrt/microsoft.ui.xaml.Shapes.Path) objects in a layout container, with move and draw commands for [**Path.Data**](/windows/windows-app-sdk/api/winrt/microsoft.ui.xaml.shapes.path.data). There may be multiple **Path** elements in the XAML so that different brushes can be applied. Many of these exporters or plug-ins were originally written for Windows Presentation Foundation (WPF)  XAML or Silverlight, but the XAML path syntax is identical with Windows Runtime XAML. Usually, you can use chunks of XAML from an exporter and paste them right into a Windows Runtime XAML page. (However, you won't be able to use a **RadialGradientBrush**, if that was part of the converted XAML, because Windows Runtime XAML doesn't support that brush.)

## Related topics

* [Draw shapes](/windows/apps/design/controls/shapes)
* [Use brushes](brushes.md)
* [**Path.Data**](/windows/windows-app-sdk/api/winrt/microsoft.ui.xaml.shapes.path.data)
