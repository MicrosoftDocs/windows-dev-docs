...
Provides a value for any XAML attribute by evaluating a reference to a resource, with additional system logic that retrieves different resources depending on the currently active theme. Similar to [{StaticResource} markup extension](staticresource-markup-extension.md), resources are defined in a [**ResourceDictionary**](/windows/windows-app-sdk/api/winrt/microsoft.ui.xaml.ResourceDictionary), and a **ThemeResource** usage references the key of that resource in the **ResourceDictionary**.
...
| Term | Description |
|------|-------------|
| key | The key for the requested resource. This key is initially assigned by the [**ResourceDictionary**](/windows/windows-app-sdk/api/winrt/microsoft.ui.xaml.ResourceDictionary). A resource key can be any string defined in the XamlName Grammar. |
...
For more info on how to define resources and properly use a [**ResourceDictionary**](/windows/windows-app-sdk/api/winrt/microsoft.ui.xaml.ResourceDictionary), including sample code, see [ResourceDictionary and XAML resource references](xaml-resource-dictionary.md).
...
The rules by which a **ThemeResource** resolves to an item in a resource dictionary are generally the same as **StaticResource**. A **ThemeResource** lookup can extend into the [**ResourceDictionary**](/windows/windows-app-sdk/api/winrt/microsoft.ui.xaml.ResourceDictionary) files that are referenced in a [**ThemeDictionaries**](/windows/windows-app-sdk/api/winrt/microsoft.ui.xaml.resourcedictionary.themedictionaries) collection, but a **StaticResource** can do that also. The difference is that a **ThemeResource** can re-evaluate at run-time and a **StaticResource** can't.
...
- Brushes, in particular colors for [**SolidColorBrush**](/windows/windows-app-sdk/api/winrt/microsoft.ui.xaml.Media.SolidColorBrush). These make up about 80% of the **ThemeResource** usages in the default XAML control templates (generic.xaml).
- Pixel values for borders, offsets, margin and padding and so on.
- Font properties such as **FontFamily** or **FontSize**.
- Complete templates for a limited number of controls that are usually system-styled and used for dynamic presentation, like [**GridViewItem**](/windows/windows-app-sdk/api/winrt/microsoft.ui.xaml.Controls.GridViewItem) and [**ListViewItem**](/windows/windows-app-sdk/api/winrt/microsoft.ui.xaml.Controls.ListViewItem).
...
**ThemeResource** usages might be seen in a series of dependent values. For example, a [**Color**](/uwp/api/Windows.UI.Color) value used by a [**SolidColorBrush**](/windows/windows-app-sdk/api/winrt/microsoft.ui.xaml.Media.SolidColorBrush) that is also a keyed resource might use a **ThemeResource** reference. But any UI properties that use the keyed **SolidColorBrush** resource would also use a **ThemeResource** reference, so that it's specifically each [**Brush**](/windows/windows-app-sdk/api/winrt/microsoft.ui.xaml.Media.Brush)-type property that's enabling a dynamic value change when the theme changes.
...
Here, the properties take a [**Brush**](/windows/windows-app-sdk/api/winrt/microsoft.ui.xaml.Media.Brush) value, and the reference to [**SolidColorBrush**](/windows/windows-app-sdk/api/winrt/microsoft.ui.xaml.Media.SolidColorBrush) resources named `ButtonBackgroundThemeBrush` and `ButtonForegroundThemeBrush` are made using **ThemeResource**.

These same properties are also adjusted by some of the visual states for a [**Button**](/windows/windows-app-sdk/api/winrt/microsoft.ui.xaml.Controls.Button). Notably, the background color changes when a button is clicked. Here too, the [**Background**](/windows/windows-app-sdk/api/winrt/microsoft.ui.xaml.controls.control.background) and [**Foreground**](/windows/windows-app-sdk/api/winrt/microsoft.ui.xaml.controls.control.foreground) animations in the visual state storyboard use [**DiscreteObjectKeyFrame**](/windows/windows-app-sdk/api/winrt/microsoft.ui.xaml.Media.Animation.DiscreteObjectKeyFrame) objects and references to brushes with **ThemeResource** as the key frame value.
...
Here the [**Color**](/windows/windows-app-sdk/api/winrt/microsoft.ui.xaml.Media.SolidColorBrush.Color) value is another **ThemeResource** reference to a system resource. If you reference a system resource, and you want it to change in response to a theme change, you should use **ThemeResource** to make the reference.

## Design-time tools support for the **{ThemeResource}** markup extension

Microsoft Visual Studio can include possible key values in the Microsoft IntelliSense dropdowns when you use the **{ThemeResource}** markup extension in a XAML page. For example, as soon as you type "{ThemeResource", any of the resource keys from the [XAML theme resources](/windows/apps/design/style/xaml-theme-resources) are displayed.
