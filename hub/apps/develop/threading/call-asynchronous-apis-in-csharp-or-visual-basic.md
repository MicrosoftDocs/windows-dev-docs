...
description: Windows includes many asynchronous APIs to ensure that your app remains responsive when it does work that might take an extended amount of time.
...
Windows includes many asynchronous APIs to ensure that your app remains responsive when it does work that might take an extended amount of time. This topic discusses how to use asynchronous methods in C# or Microsoft Visual Basic.

Asynchronous APIs keep your app from waiting for large operations to complete before continuing execution. For example, an app that downloads info from the Internet might spend several seconds waiting for the info to arrive. If you use a synchronous method to retrieve the info, the app is blocked until the method returns. The app won't respond to user interaction and because it seems non-responsive, the user might become frustrated. By providing asynchronous APIs, Windows helps to ensure that your app stays responsive to the user when it's performing long operations.

Most of the asynchronous APIs in Windows don't have synchronous counterparts, so you need to be sure to understand how to use the asynchronous APIs with C# or Visual Basic in your Windows app. Here we show how to call asynchronous APIs.
...
Suppose that you have an app that lists the titles of blog posts from a certain location. The app has a [**Button**](/windows/windows-app-sdk/api/winrt/microsoft.ui.xaml.controls.button) that the user clicks to get the titles. The titles are displayed in a [**TextBlock**](/windows/windows-app-sdk/api/winrt/microsoft.ui.xaml.controls.textblock). When the user clicks the button, it is important that the app remains responsive while it waits for the info from the blog's website. To ensure this responsiveness, Windows provides an asynchronous method, [**SyndicationClient.RetrieveFeedAsync**](/uwp/api/windows.web.syndication.syndicationclient.retrievefeedasync), to download the feed.
...
When you use an asynchronous method, you can examine the signature to see what you'll get back after awaiting the value returned from the method. All asynchronous APIs return one of the following types:
...
> [!NOTE]
> `FileOpenPicker` and other picker APIs require window handle (HWND) initialization in WinUI 3 desktop apps. Before calling `PickSingleFileAsync`, call `WinRT.Interop.InitializeWithWindow.Initialize(picker, hwnd)`, where `hwnd` comes from `WindowNative.GetWindowHandle(this)`. See [Display WinUI 3 picker APIs that require a window handle (HWND)](/windows/apps/develop/ui-input/display-ui-objects).

Asynchronous methods that are defined in **.NET for Windows apps** have the return type [**Task**](/dotnet/api/system.threading.tasks.task) or [**Task&lt;TResult&gt;**](/dotnet/api/system.threading.tasks.task-1). Methods that return **Task** are similar to the asynchronous methods that return [**IAsyncAction**](/uwp/api/windows.foundation.iasyncaction). In each case, the result of the asynchronous method is **void**. The return type **Task&lt;TResult&gt;** is similar to [**IAsyncOperation&lt;TResult&gt;**](/uwp/api/Windows.Foundation.IAsyncOperation_TResult_) in that the result of the asynchronous method when running the task is the same type as the `TResult` type parameter.
...
As you encounter asynchronous methods, it is important to remember:
