...
description: This article provides an overview of Bluetooth Generic Attribute Profile (GATT) Server for Windows apps, along with sample code for common use cases.
ms.date: 05/27/2026
ms.topic: article
...
This topic demonstrates how to use the Bluetooth Generic Attribute (GATT) Server APIs for Windows apps.
...
> [!CAUTION]
> `Broadcast` is not supported at this time. Specifying the `Broadcast` GattCharacteristicProperty will result in an exception.
...
When a service is published in the foreground, an application must call StopAdvertising when the application suspends.

## Respond to Read and Write requests

As see previously while declaring the required characteristics, GattLocalCharacteristics have 3 types of events - `ReadRequested`, `WriteRequested` and `SubscribedClientsChanged`.
