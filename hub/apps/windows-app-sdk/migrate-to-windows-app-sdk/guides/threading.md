...
The PDE debugger extension (for the `!pde.dse` command) is a community-provided WinDbg extension. Search for 'pde WinDbg extension' to find the current download location. Put the appropriate x64 or x86 `.dll` from the package into the `winext` directory of your WinDbg install, and then `!pde.dse` will work on stowed exception crash dumps.
