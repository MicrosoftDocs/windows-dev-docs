...
| [InkCanvas](/uwp/api/windows.ui.xaml.controls.inkcanvas) | ⚠️ Experimental only (introduced in 2.0 Experimental 1). Not available in the stable 2.0 channel. [InkToolbar](/uwp/api/windows.ui.xaml.controls.inktoolbar) is not available. See [Known control gaps](#known-control-gaps). If you reference `InkCanvas` or `InkToolbar` in XAML, you get a `WMC0001 Unknown type` build error because these types don't exist in the `Microsoft.UI.Xaml.Controls` namespace. |
| [Windows.Data.Html.HtmlUtilities](/uwp/api/windows.data.html.htmlutilities) | ❌ Calls into the IE/Trident engine (`iertutil.dll`), which fail-fasts with `STATUS_STACK_BUFFER_OVERRUN` (`0xc0000409`) in packaged Windows App SDK apps. The same crash occurs if you use the legacy UWP [WebView](/uwp/api/windows.ui.xaml.controls.webview) control. Use [WebView2](/microsoft-edge/webview2/) for HTML rendering, or a managed HTML-to-text library (such as [HtmlAgilityPack](https://html-agility-pack.net/)) for `ConvertToText` scenarios. |
| [CaptureElement](/uwp/api/windows.ui.xaml.controls.captureelement) | ❌ No WinUI 3 equivalent. See [Known control gaps](#known-control-gaps). |
| [RadialController](/uwp/api/windows.ui.input.radialcontroller) interop | ❌ `IRadialControllerInterop.CreateForWindow` fail-fasts at runtime in packaged WinUI 3 apps despite being documented as desktop-supported. No workaround available; see [Known control gaps](#known-control-gaps). |
...
| [CaptureElement](/uwp/api/windows.ui.xaml.controls.captureelement) | Not available | Use [MediaPlayerElement](/windows/windows-app-sdk/api/winrt/microsoft.ui.xaml.controls.mediaplayerelement) with [MediaSource.CreateFromMediaFrameSource](/uwp/api/windows.media.core.mediasource.createfrommediaframesource) for camera preview scenarios |
| [RadialController](/uwp/api/windows.ui.input.radialcontroller) | Runtime failure in packaged apps | No first-party alternative; implement custom dial UI or use Win32 raw input |
