...
description: If your app isn't installed by using MSIX (that is, it's packaged with external location or unpackaged), then you must initialize the Windows App SDK for use before you can call Windows App SDK features such as WinUI, App Lifecycle, MRT Core, and DWriteCore.
ms.topic: article
ms.date: 09/11/2026
...
* Beginning in Windows App SDK 1.0, that can be done automatically when your app starts via auto-initialization (set the project property `<WindowsPackageType>None</WindowsPackageType>`). For a demonstration, see [Create and run a WinUI 3 project](../get-started/start-here.md).
...
For details about the initialization options, see [MddBootstrapInitializeOptions](/windows/windows-app-sdk/api/win32/mddbootstrap/ne-mddbootstrap-mddbootstrapinitializeoptions).
