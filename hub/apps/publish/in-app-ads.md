...
To create an ad unit for a [banner ad](/windows/uwp/monetize/banner-ads), [interstitial ad](/windows/uwp/monetize/interstitial-ads), or [native ad](/windows/uwp/monetize/native-ads) in your app:
...
6. Configure the following additional settings as desired:
    ...
    * If you select the **PC/Tablet (Windows 8.1)** or **Mobile (Windows Phone 8.x)** device family for a banner ad unit, you can optionally select **Show community ads in your app** to opt in to [community ads](/windows/uwp/monetize/index).
...
|  Ad network  |  Description  |  Supported ad types  |
|--------------|---------------|---------------------|
| Microsoft Community ads |  If you [create a promotional ad campaign for one of your apps](/windows/uwp/monetize/index) and configure this campaign as a [community ad campaign](/windows/uwp/monetize/index), select this options to show ads from this campaign. | Banner, Banner interstitial |
...
The available ad networks serve ads in all [supported markets](publish-your-app/market-selection.md?pivots=store-installer-msix#microsoft-store-consumer-markets), with the following exceptions.
...
When you [create an ad unit](#create-ad-unit) or [select an existing ad unit](#available-ad-units), the **COPPA compliance** section appears at the bottom of the page if the selected app for the ad unit has at least one submission that has reached the [in the Store](publish-your-app/app-certification-process.md?pivots=store-installer-msix#in-the-store) step in the app certification process.
