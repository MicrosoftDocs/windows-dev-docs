...
* The **uap10:HostRuntimeDependency** element. This is a child of the [Dependencies](https://docs.microsoft.com/uwp/schemas/appxpackage/uapmanifestschema/element-dependencies) element.
...
| **uap10:HostRuntimeDependency** | This element element declares a dependency on the host app package. This consists of the **Name** and **Publisher** of the host package, and the **MinVersion** it depends on. These values can be found under the [Identity](https://docs.microsoft.com/uwp/schemas/appxpackage/uapmanifestschema/element-identity) element in the host package. |
...
For a fully functional sample app that declares itself as a host and then dynamically registers a hosted app package at runtime, see the [hosted app sample](https://aka.ms/hostedappsample).
