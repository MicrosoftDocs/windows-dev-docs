...
| Path property | `{x:Bind Path=a.b.c}`<br/>-<br/>`{Binding Path=a.b.c}` | In x:Bind, Path is rooted at the Page by default, not the DataContext. |
...
| Converter | `{x:Bind IsShown, Converter={StaticResource BoolToVisibility}}`<br/>-<br/>`{Binding IsShown, Converter={StaticResource BoolToVisibility}}` | Converters must be declared at the root of the Page/ResourceDictionary, or in App.xaml. |
| ConverterParameter, ConverterLanguage | `{x:Bind IsShown, Converter={StaticResource BoolToVisibility}, ConverterParameter=One, ConverterLanguage=fr-fr}`<br/>-<br/>`{Binding IsShown, Converter={StaticResource BoolToVisibility}, ConverterParameter=One, ConverterLanguage=fr-fr}` | Converters must be declared at the root of the Page/ResourceDictionary, or in App.xaml. |
...
| ElementName | `{x:Bind slider1.Value}`<br/>-<br/>`{Binding Value, ElementName=slider1}` | With {x:Bind} you're binding to a field; Path is rooted at the Page by default, so any named element can be accessed via its field. |
