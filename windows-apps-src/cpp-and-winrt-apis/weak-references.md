...
The code listing below first defines a simple **EventSource** class, which raises a generic event that's handled by any delegates that have been added to it. This example event happens to use the [**Windows::Foundation::EventHandler**](https://microsoft.github.io/windows-docs-rs/doc/windows/Foundation/struct.EventHandler.html) delegate type, but the issues and remedies here apply to any and all delegate types.
