...
3. In Toolbox, expand **Common XAML controls** and find the [TextBlock](https://docs.microsoft.com/uwp/api/windows.ui.xaml.controls.textblock) control. Drag a **TextBlock** control onto the design surface near the upper-left corner of the page.
...
4. In the **Properties** panel, change the **Name** value of the **TextBlock** control from **textBlock** to **TitleTextBlock**. (Make sure the **TextBlock** control is still the active object.)
...
First, you'll replace the root layout [Grid](https://docs.microsoft.com/uwp/api/windows.ui.xaml.controls.grid) with [RelativePanel](https://docs.microsoft.com/uwp/api/windows.ui.xaml.controls.relativepanel). **RelativePanel** makes it easier to rearrange chunks of UI relative to the panel or other pieces of UI. You'll see its usefulness in the [XAML adaptive layout](xaml-basics-adaptive-layout.md) tutorial. 
...
1. In the XAML editor, change the root **Grid** to **RelativePanel**.
    ...
    **After**
    ...
    For more info about layout using **RelativePanel**, see [Layout panels](https://docs.microsoft.com/windows/uwp/layout/layout-panels#relativepanel).
...
2. To show the rating, use the **RadRating** control from the [Telerik UI for UWP](https://github.com/telerik/UI-For-UWP) NuGet package. Add a XAML namespace reference that specifies the namespace for the Telerik controls. Put this in the opening **Page** tag, right after the other `xmlns:` entries.
...
4. In the dialog box, change the **Name (key)** value to **ImageGridView_DefaultItemTemplate**, and then select **OK**.
    ...
    - The **GridView** control's **ItemTemplate** property is set to the **DataTemplate** resource.
...
6. Add controls to the **Grid** layout.
    ...
    c. Add the **RadRating** control to the outer (vertical) **StackPanel** control. Place it after the inner (horizontal) **StackPanel** control.
...
Run the app now to see the **GridView** control with the item template that you just created. You might not see the rating control, though, because it has white stars on a white background. You'll change the background color next.
...
2. In the dialog box, change the **Name (key)** value to **ImageGridView_DefaultItemContainerStyle**, and then select **OK**.
    ...
    The **GridViewItem** default style sets a lot of properties. You should always start with a copy of the default style and modify only the necessary properties. Otherwise, the visuals might not show up the way you expect because some properties won't be set correctly.
...
Run the app and see how it looks now. Resize the app window. The **GridView** control takes care of rearranging the images for you, but at some widths, there's a lot of space on the right side of the app window. It would look better if the images were centered. We'll take care of that next.
...
To center the images on the page, you need to adjust the alignment of the **Grid** control on the page. Or do you need to adjust the alignment of the images in **GridView**? Does it matter? Let's see.
...
1. In **GridView**, set the **HorizontalAlignment** property to **Center**.
...
5. In the dialog box, change the **Name (key)** value to **ImageGridView_ItemsPanelTemplate**, and then select **OK**.
    ...
    Just as you've used various panels to lay out the controls in your app, **GridView** has an internal panel that manages the layout of its items. Now that you have access to this panel (**ItemsWrapGrid**), you can modify its properties to change the layout of items inside the **GridView** control.

6. In **ItemsWrapGrid**, set the **HorizontalAlignment** property to **Center**.
