...
|    Very Low     |    Yes – user can block all notifications from an app.        No – It cannot prevent very low priority   notifications from being throttled in battery saver mode    |    This is almost the same as low priority except users cannot ovveride the battery saver policy. These notifications will never   be delivered in battery saver.    |    Syncing files for a sync service.    |
