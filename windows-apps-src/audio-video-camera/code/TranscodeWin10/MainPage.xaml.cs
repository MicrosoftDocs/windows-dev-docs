...
namespace TranscodeWin10
{
    ...
    public sealed partial class MainPage : Page
    {
        ...
        private async System.Threading.Tasks.Task<bool> IsVideoDecoderSupportedAsync(string codecSubtype)
        {
            var codecQuery = new CodecQuery();
            IReadOnlyList<CodecInfo> result = await codecQuery.FindAllAsync(CodecKind.Video, CodecCategory.Decoder, codecSubtype);
        ...
        private async void H264Button_Click(object sender, RoutedEventArgs e)
        {
            var codecQuery = new CodecQuery();
            IReadOnlyList<CodecInfo> result = await codecQuery.FindAllAsync(CodecKind.Video, CodecCategory.Decoder, "H264");

            if (result.Count > 0)
                H264ResultTextBox.Text = result.ToString();
        }

        private async void HEVCButton_Click(object sender, RoutedEventArgs e)
        {
            var result = await IsVideoDecoderSupportedAsync(
        }

        private async void FindAllCodecsButton_Click(object sender, RoutedEventArgs e)
        {
            var codecQuery = new CodecQuery();
            IReadOnlyList<CodecInfo> result = await codecQuery.FindAllAsync(CodecKind.Video, CodecCategory.Encoder, "");
            IEnumerable<CodecInfo> h264Codecs = result.Where(c => c.Subtypes.Contains(CodecSubtypes.VideoFormatH264));



            foreach (var codecInfo in h264Codecs)
            {
                FindAllResultsTextBox.Text += $"{codecInfo.DisplayName} | {string.Join("|",codecInfo.Subtypes.ToArray())}\n";
                
            }

            VideoEncodingProperties props = new VideoEncodingProperties();
            props.Subtype = CodecSubtypes.VideoFormatH264;
