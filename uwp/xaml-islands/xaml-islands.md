...
title: Host UWP XAML controls in desktop apps
ms.date: 10/20/2020
ms.topic: how-to
...
# Host UWP XAML controls in desktop apps (UWP XAML Islands)
...
Starting in Windows 10, version 1903, you can host UWP XAML controls in non-UWP desktop applications using a feature called *UWP XAML Islands*. This feature lets you enhance the look, feel, and functionality of your existing WPF, Windows Forms, and C++ desktop (Win32) applications with Windows UI features that are only available via UWP XAML controls. This means that you can use UWP features such as [Windows Ink](/windows/uwp/design/input/pen-and-stylus-interactions) and controls that support the [Fluent Design System](/windows/uwp/design/fluent-design-system/index) in your existing WPF, Windows Forms, and C++ desktop applications.

You can host any UWP XAML control that derives from [Windows.UI.Xaml.UIElement](/uwp/api/windows.ui.xaml.uielement), including:

* Most first-party UWP XAML controls provided by the Windows SDK or the WinUI for UWP library (see [exceptions](#limitations-and-workarounds)).
* Any custom UWP XAML control (for example, a user control that consists of several UWP XAML controls that work together). You must have the source code for the custom control so you can compile it with your application.
...
The way you use UWP XAML Islands depends on your application type and the types of UWP XAML controls you want to host.

## Requirements

UWP XAML Islands have these run time requirements:
...
We recommend that WPF and Windows Forms applications use the XAML Island .NET controls that are available in the Windows Community Toolkit. These controls provide an object model that mimics (or provides access to) the properties, methods, and events of the corresponding UWP XAML controls. They also handle behavior such as keyboard navigation and layout changes.

There are two sets of XAML Island controls for WPF and Windows Forms applications: *wrapped controls* and *host controls*. 

### Wrapped controls

WPF and Windows Forms applications can use a selection of XAML Island controls that wrap the interface and functionality of a specific UWP XAML control. You can add these controls directly to the design surface of your WPF or Windows Forms project and then use them like any other WPF or Windows Forms control in the designer.

The following wrapped UWP XAML controls are currently available in the Windows Community Toolkit. 
...
For a walkthrough that demonstrates how to use the wrapped UWP XAML controls, see [Use XAML Islands to host a UWP XAML control in a C# WPF app](host-standard-control-with-xaml-islands.md).
...
| Control | Minimum supported OS | Description |
|-----------------|-------------------------------|-------------|
| [WindowsXamlHost](/windows/communitytoolkit/controls/wpf-winforms/windowsxamlhost) | Windows 10, version 1903 | Can host any UWP XAML control that derives from [Windows.UI.Xaml.UIElement](/uwp/api/windows.ui.xaml.uielement), including any first-party UWP XAML control provided by the Windows SDK as well as custom controls. |

For walkthroughs that demonstrate how to use the **WindowsXamlHost** control, see [Use XAML Islands to host a UWP XAML control in a C# WPF app](host-standard-control-with-xaml-islands.md) and [Host a custom UWP XAML control in a WPF app using XAML Islands](host-custom-control-with-xaml-islands.md).
...
* If you're hosting a custom UWP XAML control, you'll also need to perform some additional steps to reference the custom control. For more info, see [Host a custom UWP XAML control in a WPF app using XAML Islands](host-custom-control-with-xaml-islands.md).
...
The UWP XAML hosting API consists of several Windows Runtime classes and COM interfaces that your C++ desktop application can use to host any UWP XAML control that derives from [Windows.UI.Xaml.UIElement](/uwp/api/windows.ui.xaml.uielement). You can host UWP XAML controls in any UI element in your application that has an associated window handle (HWND). For more information about this API, see the following articles.

* [Using the UWP XAML hosting API in a C++ desktop app](using-the-xaml-hosting-api.md)
* [Host a standard UWP XAML control in a C++ desktop app](host-standard-control-with-xaml-islands-cpp.md)
...
## Architecture of UWP XAML Islands
...
:heavy_check_mark: Using `x:Bind` with hosted controls in UWP XAML Islands is not supported. You'll have to declare the data model in a .NET Standard library.

### Not supported

:no_entry_sign: Using UWP XAML Islands in WPF and Windows Forms apps that target the .NET Framework. UWP XAML Islands are supported only in apps that target .NET Core 3.x.

:no_entry_sign: UWP XAML content in UWP XAML Islands doesn't respond to Windows theme changes from dark to light or vice versa at run time. Content does respond to high contrast changes at run time.
...
For a table of common windowing-related APIs that you should avoid in the context of UWP XAML Islands and the recommended [XamlRoot](/uwp/api/windows.ui.xaml.xamlroot) replacements, see the table in [this section](/windows/uwp/design/layout/show-multiple-views#make-code-portable-across-windowing-hosts).
...
For more background information and tutorials about using UWP XAML Islands, see the following articles and resources:

* [UWP XAML Islands code samples](https://github.com/microsoft/Xaml-Islands-Samples): This repo contains Windows Forms, WPF, and C++ desktop (Win32) samples that demonstrate how to use UWP XAML Islands.
* [UWP XAML Islands v1 - Updates and Roadmap](https://blogs.windows.com/windowsdeveloper/2019/06/13/xaml-islands-v1-updates-and-roadmap): This blog post discusses many common questions about UWP XAML Islands and provides a detailed development roadmap.
